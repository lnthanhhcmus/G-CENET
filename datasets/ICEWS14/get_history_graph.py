...
def get_history_target(quadruples, s_history_event_o, o_history_event_s):
    '''
    quadruples: tập các facts \n
    s_history_event_o: subject --> relation --> object \n
    o_history_event_s: object --> relation --> subject \n
    '''
    s_history_oid = []
    o_history_sid = []
    ss = quadruples[:, 0]
    ...
    # duyệt tất cả facts trong index
    for ix in tqdm.tqdm(range(quadruples.shape[0])):
        s_history_oid.append([])
        o_history_sid.append([])

        # duyệt tất cả relation có liên quan tới facts theo subject
        ...
        # duyệt tất cả các relation có liên quan tới facts theo object
